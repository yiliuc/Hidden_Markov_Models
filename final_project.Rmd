...
# Compute number of parameters: k = m^2 + m - 1
num_params <- states^2 + states - 1

# Compute log-likelihood: logLik = -0.5 * (AIC - 2k)
...
# Assume `results` and `states_prediction` are already defined in your environment

# Format mean + CI
summary_col <- with(results, sprintf("%.2f (%.2f, %.2f)", mean, lower_95, upper_95))

# Transpose state prediction matrix for day-wise alignment
state_probs <- t(states_prediction)

# Format state probability row: "S1: 0.76, S2: 0.67, ..."
...
# Output with kable
kable(summary_table, caption = "5-Day Forecast Summary with State Probabilities")
```
...
The above table shows the model diagonistics for different states, we can see that the two states is enough.

By performing the EM Algorithm with two states, the estimated initial distribution $\boldsymbol\delta=(1, 0)$, and the estimated transition matrix is
$$
...
The estimated parameters are $\mu_1=0.019,\sigma_1=0.013$ and $\mu_2=0.059,\sigma_2=0.033$.
...
# Overlay the expected lambda values per decoded state
points(1:150, tail(decoded_means, 150), pch = 19, cex = 0.3)
